...
class DiagnosticAgent:
    def __init__(self, api_key=None):
        ...
        if not self.llm_available and self.groq_api_key and self.groq_api_key.startswith("gsk-"):
            try:
                self.llm = ChatGroq(temperature=0.1, model_name="mixtral-8x7b-32768", api_key=self.groq_api_key)
                # Simple test
                self.llm.invoke("test")
                self.llm_available = True
    ...
    def analyze_case(self, clinical_case, clinical_notes, lab_data, image_findings, documents, document_types=""):
        ...
        """
        # If the LLM is not available, return an error immediately.
        if not self.llm_available:
            return {
                "error": self.llm_error_message,
                "diagnostic_analysis": f"**Analysis Failed:** Could not connect to the AI model. {self.llm_error_message}",
                "formatted_report": "**Analysis Failed**",
                "summary": "Analysis failed due to LLM unavailability."
            }
